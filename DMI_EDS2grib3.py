...
parser.add_argument("--bbox", required=False, default="9.29,54.54,13.04,56.19", help="Bounding box of area to download.")
parser.add_argument("--out-file", required=False, default=file_path, help="The location of the output file.")
...
def interpolate_missing_values(lons, lats, values):
    """
    Interpolate missing (0.0) values in the wind data using nearest-neighbor extrapolation.
    """
    points = np.array(list(zip(lons, lats)))
    values = np.array(values)

    # Find points where values are 0 (assumed missing)
    mask = values != 0

    # Perform nearest-neighbor interpolation for missing values
    interpolated_values = griddata(points[mask], values[mask], points, method='nearest')

    # Replace NaN values with 0.0 (default value)
...
if geojson_data:
    ...
    # Step 5: Interpolate and create a GRIB2 file with steps
    with open(file_path, "wb") as grib_file:
        for step_time, data in data_by_step.items():
            ...
            # Interpolate missing values for wind-u and wind-v
            interpolated_wind_u = interpolate_missing_values(data["lons"], data["lats"], data["wind_u"])
            interpolated_wind_v = interpolate_missing_values(data["lons"], data["lats"], data["wind_v"])
            ...
            ec.codes_set_values(gid_u, interpolated_wind_u)
            ...
            ec.codes_set_values(gid_v, interpolated_wind_v)
