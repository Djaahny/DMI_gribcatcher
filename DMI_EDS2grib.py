...
url = f"https://dmigw.govcloud.dk/v1/forecastedr/collections/dkss_nsbs/cube?bbox=9.29,54.54,13.04,56.19&parameter-name=wind-u,wind-v&datetime={datetime_string}%2F..&crs=crs84&f=GeoJSON&api-key={args.api_key}"
...
if geojson_data:
    ...
    with open("output_with_steps.grib2", "wb") as grib_file:
        for step_time, data in data_by_step.items():
            ...
            refTime = (forecast_time.strftime("%H%M").zfill(4))
            date_time_int = int(refTime)
            forecastTime = i
            i = i + 1
            ...
            #ec.codes_set(gid_u, "dataTime", 1) #Set the data time
